...
contract BnbStaking is Ownable {
    ...
    // The block number when ZAI mining starts.
    uint256 public startBlock;
    // The block number when ZAI mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 zaiReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accZaifPerShare = accZaifPerShare.add(zaiReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 zaiReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accZaifPerShare = pool.accZaifPerShare.add(zaiReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
